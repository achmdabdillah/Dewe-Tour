...
const ListTransaction = () => {
    ...
    const updateData = async (id, status, qty) => {
        try {
            ...
            if(status === 'Approved' && (detailModal.quotaFilled + qty) <= detailModal.quota){
                await API.patch(`/transactions/${id}`, statusUpdate, config);

                const addQuota = JSON.stringify({quotaFilled: detailModal?.quotaFilled + qty})
                await API.patch(`/trips/${detailModal?.idTrip}`, addQuota, config);
            
            } else if(status === 'Cancelled' && (detailModal.quotaFilled + qty) <= detailModal.quota){
                await API.patch(`/transactions/${id}`, statusUpdate, config);
            } else {
           ...
           <div className="nav-container list-margin-top">
              <div className="d-flex flex-column">
                ...
                <table className="transaction-tbl">
                        ...
                        {data.filter(item => item?.status !== 'Waiting Payment').reverse().map((item, i) => (
                            <>
                                <tr>
                                    <td>{i + 1}</td>
                                    ...
                                    {/* start */}
                                    <td>
                                        <img data-bs-toggle="modal" data-bs-target="#proofModal" onClick={() => {proofModal(item?.attachment); }} className='box-sm pointer' src={item?.attachment} alt="no payment" />
                                        <div className="modal fade" id="proofModal">
                                            <div className="modal-dialog modal-dialog-centered">
                                                <div className="modal-content">
                                                    <img className='paymentProofPic img-fluid' src={proof} alt="no payment" />
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                   {/* ends */}
                                    <td><span className={
                                        item?.status === 'Approved' ? "roboto-bold status-green" : 
                                        item?.status === 'Cancelled' ? "roboto-bold status-red" : "roboto-bold status-orange"
                                        } >{item?.status}</span></td>
                                    <td className="pointer" data-bs-toggle="modal" data-bs-target="#exampleModal"><img src="/assets/lup.png" alt="" onClick={() => handleModal(item)}/></td>
                                </tr>
                            </>
                        ))}
                </table>
                ...
                <div className="modal-dialog custom">
                    ...
                    <div className="payment-ticket" >
                            <div className="payment-container">
                                ...
                                <div className="payment-body d-flex">
                                    <div className="pbody-title">
                                        ...
                                        <p><span className={
                                    detailModal?.status === "Approved" ? "payment-status status-green bg-green" :
                                    detailModal?.status === "Cancelled" ? "payment-status status-red bg-red" :
                                    "payment-status status-orange bg-orange" 
                                    }>{detailModal.status}</span></p>
                                    </div>
                                    <div className="pbody-center container">
                                        <div className="row row-cols-2">
                                            ...
                                            <div className="col pbody-info">
                                                <h1>Duration</h1>
                                                <p>{detailModal.day} Day {detailModal.night} Night</p>
                                            </div>
                                            <div className="col pbody-info">
                                                <h1>Accomodation</h1>
                                    ...
                                    <div className="pbody-end">
                                        <label for="proof">
                                            <img className="box" src={detailModal.attachment ? detailModal.attachment : "/assets/struk.png" } alt="" />
                                        </label>
                                        <input type="file" id="proof" name="attachment" className="filestyle"></input>
                                ...
                                <div className="payment-end">
                                    <div className="payment-table">
                                        ...
                                        <tr className="t-end">
                                            ...
                                            <td className="avenir red">{formatter(detailModal.total)}</td>
                                        </tr>
                                        </table>
                                    </div>
                                </div>
                        ...
                        (<div className="d-flex end mt-2 modal-approve">
                            <div>
                                <button data-bs-dismiss="modal" className="modal-action-btn me-5 avenir action-red" onClick={() => {
                                    updateData(detailModal?.id, 'Cancelled', detailModal.qty);
                                    }}>Cancel</button>
                                <button data-bs-dismiss="modal" className="modal-action-btn avenir action-green" onClick={() => {
                                    updateData(detailModal?.id, 'Approved', detailModal.qty);
                                    }}>Approve</button>
                            </div>
                        </div>) 
                        : 
